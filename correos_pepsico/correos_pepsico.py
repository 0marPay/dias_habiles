...
def obtener_archivo():
    ...
    archivo = int(input("Ingrese el número correspondiente al ARCHIVO: "))
    archivo = archivos[archivo]
    print("Ha seleccionado el archivo:", archivo); print()
    return archivo

def obtener_hoja(archivo):
    # Leer las hojas del archivo Excel
    xls = pd.ExcelFile(archivo)
    hojas = xls.sheet_names
    print("Hojas en el archivo:"); print_lista(hojas)

    hoja = int(input("Ingrese el número correspondiente a la HOJA: "))
    hoja = hojas[hoja]
    print("Ha seleccionado la HOJA:", hoja); print()
    return hoja

def obtener_datos(archivo, hoja):
...
if __name__ == '__main__':
    ...
    while True:
        print()
        input("Ingrese cualquier tecla para iniciar")
        print()
        try:
            archivo = obtener_archivo()
            hoja = obtener_hoja(archivo)
            df = obtener_datos(archivo, hoja)
